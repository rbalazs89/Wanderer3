...
public class NPC_Child1 extends NPC {
    ...
    public void getImage() {
        ...
        for (int i = 0; i < maxWalkSpriteArrayLength; i++) {
            //down
            walkImages[2][i] = setupSheet("/entity/npckid/childsprites", i * 48,0, 48, 48, 64, 64);
        }
        for (int i = 0; i < maxWalkSpriteArrayLength; i++) {
            //left
            walkImages[3][i] = setupSheet("/entity/npckid/childsprites", i * 48, 48,48, 48, 64, 64);
        }
        for (int i = 0; i < maxWalkSpriteArrayLength; i++) {
            //up
            walkImages[0][i] = setupSheet("/entity/npckid/childsprites",  i * 48, 144,48, 48, 64, 64);
        }
            //right
        for (int i = 0; i < maxWalkSpriteArrayLength; i++) {
            walkImages[1][i] = setupSheet("/entity/npckid/childsprites",  i * 48, 96,48, 48, 64, 64);
        }
        image = walkImages[2][1];
    }
