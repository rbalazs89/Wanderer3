...
public class NPC_Child4 extends NPC {

    public NPC_Child4(GamePanel gp) {
        super(gp);
        defaultSpeed = 1;
        speed = defaultSpeed;
    ...
    public void getImage() {
        ...
        for (int i = 0; i < maxWalkSpriteArrayLength; i++) {
            //down
            walkImages[2][i] = setupSheet("/entity/npckid/childsprites", 144 * 3 + i * 48, 0,48, 48, 64, 64);
        }
        for (int i = 0; i < maxWalkSpriteArrayLength; i++) {
            //left
            walkImages[3][i] = setupSheet("/entity/npckid/childsprites", 144 * 3 + i * 48, 48,48, 48, 64, 64);
        }
        for (int i = 0; i < maxWalkSpriteArrayLength; i++) {
            //up
            walkImages[0][i] = setupSheet("/entity/npckid/childsprites",  144 * 3 + i * 48, 144,48, 48, 64, 64);
        }
            //right
        for (int i = 0; i < maxWalkSpriteArrayLength; i++) {
            walkImages[1][i] = setupSheet("/entity/npckid/childsprites",  144 * 3 + i * 48, 96,48, 48, 64, 64);
        }
        image = walkImages[1][1];
    }
